...
class CheckInEmbedding(nn.Module):
    def __init__(self, f_embed_size, vocab_size):
        ...
        self.poi_embed = nn.Embedding(
            poi_num + 1, self.embed_size, padding_idx=poi_num)
        self.cat_embed = nn.Embedding(
            cat_num + 1, self.embed_size, padding_idx=cat_num)
        ...
        self.day_embed = nn.Embedding(
            day_num + 1, self.embed_size, padding_idx=day_num)
...
class Attention(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        # sum([len, embed_size] * [len, 1])  -> [embed_size]
...
class CLSPRec(nn.Module):
    ...
    def ssl(self, embedding_1, embedding_2, neg_embedding):
        ...
        def single_infoNCE_loss_simple(embedding1, embedding2, neg_embedding):
            ...
            con_loss = torch.sum(-torch.log(1e-8 + torch.sigmoid(pos)) - settings.neg_weight * torch.log(
                1e-8 + (one - torch.sigmoid(neg))))
            return con_loss

        ssl_loss = single_infoNCE_loss_simple(
            embedding_1, embedding_2, neg_embedding)
        return ssl_loss
    ...
    def forward(self, sample, neg_sample_list):
        ...
        if settings.enable_drop_two:
            short_term_features = short_term_sequence[0][:, :- 3]
            target = short_term_sequence[0][0, -3]
        else:
        ...
        long_term_sequences = self.feature_mask(
            long_term_sequences, settings.mask_prop)
